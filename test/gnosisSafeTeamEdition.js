...
contract('GnosisSafeTeamEdition', function(accounts) {
    ...
    let executeTransaction = async function(subject, accounts, to, value, data, operation, opts) {
        ...
        let executeDataWithoutSignatures = gnosisSafe.contract.execTransaction.getData(to, value, data, operation, 0, 0, 0, 0, 0, nonce, "0x")
        ...
        let tx = await gnosisSafe.execTransaction(to, value, data, operation, 0, 0, 0, 0, 0, nonce, sigs, {from: txSender})
        utils.logGasUsage(subject, tx)

        let executeDataUsedSignatures = gnosisSafe.contract.execTransaction.getData(to, value, data, operation, 0, 0, 0, 0, 0, nonce + 1, sigs)
