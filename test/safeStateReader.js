...
contract('Gas Estimation', function(accounts) {
    ...
    it('should be able to read state', async () => {
        // Create lightwallet
        ...
        const resp = await web3.eth.call({
            ...
            data: await gnosisSafe.contract.methods.requestState(gnosisSafeStateReader.address, gnosisSafeStateReader.contract.methods.getOwners().encodeABI()).encodeABI()
        })
        console.log({resp})
        ...
        const resp3 = await web3.eth.call({
            to: gnosisSafe.address, 
            data: await gnosisSafe.contract.methods.requestState(utils.Address0, gnosisSafeStateReader.contract.methods.isOwner(lw.accounts[0]).encodeABI()).encodeABI()
        })
        console.log({resp3})
        const resp4 = await web3.eth.call({
            to: gnosisSafe.address, 
            data: await gnosisSafe.contract.methods.requestState(gnosisSafeStateReader.address, gnosisSafeStateReader.contract.methods.isOwner(lw.accounts[0]).encodeABI()).encodeABI()
        })
        console.log({resp4})
        ...
        console.log({resp2})
        console.log(await testReader.checkOwner(gnosisSafe.address, lw.accounts[0]))
    })
})
